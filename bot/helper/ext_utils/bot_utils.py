...
from .db_handler import DbManger
from ..themes import BotTheme
from ...version import get_version
from ... import OWNER_ID, bot_name, bot_cache, DATABASE_URL, LOGGER, get_client, aria2, download_dict, download_dict_lock, botStartTime, user_data, config_dict, bot_loop, extra_buttons, user
...
class EngineStatus:
    def __init__(self):
        ...
        self.STATUS_TG = f"notygram v{version_cache['pyro']}"
...
def is_embed_discovery_url(url):
    ...
    return any(x in path for x in ('-episode-', '/episode-', '/ep-', '-ep-', '/stream/', '/embed/', '/porn/', '/video/', '/videos/')) or any(x in low for x in ('?ep=', '&ep='))
...
async def get_stats(event, key="home"):
    ...
    if key == "home":
        ...
        official_v = (await cmd_exec("curl -o latestversion.py https://github.com/IamElite/D2/raw/refs/heads/arnv1/bot/version.py -s && python3 latestversion.py && rm latestversion.py", True))[0]
