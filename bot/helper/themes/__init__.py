...
from ... import config_dict, LOGGER
from . import syntax_minimal

AVL_THEMES = {}
for theme in listdir('bot/helper/themes'):
    if theme.startswith('syntax_') and theme.endswith('.py'):
...
def BotTheme(var_name, **format_vars):
    ...
    if theme_ in AVL_THEMES:
        text = getattr(AVL_THEMES[theme_].SyntaXStyle(), var_name, None)
        if text is None:
