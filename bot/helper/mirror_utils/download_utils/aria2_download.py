...
from .... import aria2, download_dict_lock, download_dict, LOGGER, config_dict, aria2_options, aria2c_global, non_queued_dl, queue_dict_lock, bot_cache
...
async def add_aria2c_download(link, path, listener, filename, header, ratio, seed_time):
    ...
    if _bt_link(link):
        ...
        if bot_cache.get('trackers'):
            a2c_opt["bt-tracker"] = bot_cache['trackers']
