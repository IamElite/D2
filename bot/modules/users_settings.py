...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            'lprefix': ['Leech Filename Prefix is the Front Part attacted with the Filename of the Leech Files.', 'Send Leech Filename Prefix. Documentation Here : <a href="https://t.me/SyntaxRealm">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'lsuffix': ['Leech Filename Suffix is the End Part attached with the Filename of the Leech Files', 'Send Leech Filename Suffix. Documentation Here : <a href="https://t.me/SyntaxRealm">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'lremname': ['Leech Filename Remname is combination of Regex(s) used for removing or manipulating Filename of the Leech Files', 'Send Leech Filename Remname. Documentation Here : <a href="https://t.me/SyntaxRealm">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'lcaption': ['Leech Caption is the Custom Caption on the Leech Files Uploaded by the bot', 'Send Leech Caption. You can add HTML tags. Documentation Here : <a href="https://t.me/SyntaxRealm">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            ...
            'usess': [f'User Session is Telegram Session used to Download Private Contents from Private Channels with no compromise in Privacy, Build with Encryption.\n{"<b>Warning:</b> This Bot is not secured. We recommend asking the group owner to set the Upstream repo to the Official repo. If it is not the official repo, then Syntax Realm is not responsible for any issues that may occur in your account." if config_dict["UPSTREAM_REPO"] != "https://github.com/IamElite/D2" else "Bot is Secure. You can use the session securely."}', 'Send your Session String.\n<b>Timeout:</b> 60 sec'],
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        buttons.ibutton("Add Custom Tag", f"userset {user_id} md_cman", "l_body")
...
async def add_custom_md_btn(client, message, pre_event):
    user_id = message.from_user.id
    handler_dict[user_id] = False
    label = (message.text or '').strip()
    await deleteMessage(message)
    if label:
        label = re_sub(r'\s+', ' ', label.replace(':', ' ').replace('|', ' '))[:32].strip()
        labels = get_custom_btns(user_data.get(user_id, {}))
        if label and len(labels) < 10 and label.lower() not in [l.lower() for l in labels]:
            labels.append(label)
            update_user_ldata(user_id, 'md_custom', '|'.join(labels))
    await update_user_settings(pre_event, 'metadata_menu')
    if DATABASE_URL:
        await DbManger().update_user_data(user_id)
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await query.answer()
        text = "✨ <b><u>Set All Metadata Fields</u></b>\n\nSend a single value to apply to <b>ALL</b> fields at once (e.g., `@SyntaxRealm`).\n\n<b>Timeout:</b> 60 sec"
        buttons = ButtonMaker()
        ...
        await query.answer(f"{mkey} removed!")
        await update_user_settings(query, 'metadata_menu')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
        ...
        state = f"➲ Current Value: <code>{escape(trun(cur, 60))}</code>" if cur else "➲ Status: <i>Not Set</i>"
        text = f"<b><u>{escape(mkey)}</u></b>\n\n{state}"
        mbuttons = ButtonMaker()
        mbuttons.ibutton(f"{'Change' if cur else 'Set'}", f"userset {user_id} md_xset {mkey}")
        if cur:
            mbuttons.ibutton("Remove", f"userset {user_id} md_xrm {mkey}")
        mbuttons.ibutton("Back", f"userset {user_id} metadata")
        await editMessage(message, text, mbuttons.build_menu(2))
    elif data[2] == 'md_xset':
        await query.answer()
        mkey = ' '.join(data[3:])
        text = f"⚙️ <b><u>Set: {escape(mkey)}</u></b>\n\nSend the value for <b>{escape(mkey)}</b>.\n\n<b>Timeout:</b> 60 sec"
        mbuttons = ButtonMaker()
        ...
        await editMessage(message, text, mbuttons.build_menu(1))
        pfunc = partial(set_metadata_key, pre_event=query, mkey=label)
        rfunc = partial(update_user_settings, query, 'metadata_menu')
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] == 'md_crmval':
        idx = int(data[3])
        labels = get_custom_btns(user_dict)
        if idx >= len(labels):
            return await update_user_settings(query, 'metadata_menu')
        label = labels[idx]
        meta_dict = parse_metadata_str(user_dict.get('metadata', ''))
        meta_dict.pop(label, None)
        update_user_ldata(user_id, 'metadata', '|'.join([f"{k}:{v}" for k, v in meta_dict.items() if v]))
        await query.answer(f"Value of {label} removed!")
        await update_user_settings(query, 'metadata_menu')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] == 'md_crmbtn':
        idx = int(data[3])
        labels = get_custom_btns(user_dict)
        if idx >= len(labels):
            return await update_user_settings(query, 'metadata_menu')
        ...
        meta_dict.pop(label, None)
        update_user_ldata(user_id, 'metadata', '|'.join([f"{k}:{v}" for k, v in meta_dict.items() if v]))
        await query.answer(f"Button {label} removed!")
        await update_user_settings(query, 'metadata_menu')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] == 'md_cadd':
