...
def _collect_premium_emojis(reply):
    """Scan reply for CUSTOM_EMOJI in text/caption + buttons icon_custom_emoji_id. Returns ordered dict {id: fallback}."""
    out = {}
...
async def _resolve_alt(client, cid, fallback):
    """Resolve Telegram alt via get_custom_emoji_stickers; fallback must exactly match alt or Telegram ignores entity."""
    try:
        if hasattr(client, 'get_custom_emoji_stickers'):
            stickers = await client.get_custom_emoji_stickers([int(cid)])
            if stickers:
                alt = getattr(stickers[0], 'emoji', None)
...
async def universal_id(client, message):
    ...
    try:
        ...
        if premium:
            ...
            # Resolve alt for each to avoid Telegram ignoring due to mismatch (critical: fallback must == documentAttributeCustomEmoji.alt)
            resolved = {}
            for cid, fb in premium.items():
                alt = await _resolve_alt(client, cid, fb)
                # Ensure exactly one emoji char; if alt is longer, take first grapheme (WZGram expects length 2 for most)
                resolved[cid] = alt
