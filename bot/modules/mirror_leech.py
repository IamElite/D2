...
from ..helper.mirror_utils.download_utils.telegram_download import TelegramDownloadHelper
from ..helper.telegram_helper.bot_commands import BotCommands
from ..helper.telegram_helper.filters import CustomFilters
from ..helper.telegram_helper.button_build import ButtonMaker
from ..helper.telegram_helper.message_utils import sendMessage, editMessage, editReplyMarkup, deleteMessage, get_tg_link_content, delete_links, auto_delete_message, open_category_btns, open_dump_btns
...
async def _mirror_leech(client, message, isQbit=False, isLeech=False, sameDir=None, bulk=[], multi_tag=None):
    ...
    if reply_to:
        file_ = None
        mval = getattr(getattr(reply_to, "media", None), "value", None)
    ...
    if file_ is None:
        if not link:
            ...
            await delete_links(message)
            return
        if not is_torrent_link(link) and not is_rclone_path(link) and not is_gdrive_link(link) and not is_mega_link(link) and not is_telegram_link(link) and not await aiopath.exists(link):
